...
if( len(sys.argv) != 6 ): 
	...
	print("Usage: autoEventBuilder3.py runBegin runEnd numTerminals confFile overWrite(bool)")
...
for i in range(int(sys.argv[3])):
	...
	cmd1 = "screen -dmS run3" + str(i+100) + "  "
	print(cmd1)
	os.system(cmd1)

	if(int(vecRun[i]) == int(vecRun[i+1])-sub):
		
		cmd2 = "screen -S run3" + str(i+100) + " -X stuff ' module load exp_3-17-58 ^M'" 
		print(cmd2)
		os.system(cmd2)
		cmd2 = "screen -S run3" + str(i+100) + " -X stuff 'EventBuilder " + confFile + " -batch " + overWrite + " -run " + str(int(vecRun[i])) + "^M'" 
		print(cmd2)
		os.system(cmd2)
		continue
	...
	cmd2 = "screen -S run3" + str(i+100) + " -X stuff 'EventBuilder " + confFile + " -batch " + overWrite + " -run " + str(int(vecRun[i])) + "-" + str(int(vecRun[i+1])-sub) + "^M'" 
	print(cmd2)
	os.system(cmd2)
