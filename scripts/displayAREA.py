...
with open(filename, 'r') as fp:
    ...
    for i, line in enumerate(fp):
        ...
        if(runningCount > 9): 
            ...
            a = np.array((([np.fromstring(i[3:], dtype=float, sep=' ') for i in lines[0:]], )))
            if ( a.shape != (1,10,6) ):
                a = np.zeros((10,6))
                a[:,4] = df[4]
            df = pd.DataFrame( a.reshape(10,6) )
            df_plot.append([countDetectorTotale-1, df.iloc[9,0]])            
...
ax.set_ylabel("Area 1408 peak")
ax.set_xlabel("Clover ID", fontsize = 10)
ax.set_xlim( -1, 65 )

ax.set_title(f"Runnumber: {filenumber}")


figName = "outputPictures/outputAREA" + str(int(filenumber)) + ".png"
plt.savefig(figName)
