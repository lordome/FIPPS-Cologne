    ...
    for i, line in enumerate(fp):
        ...
        if(runningCount > 18): 
            ...
            if ( a.shape != (1,19,6) ):
                a = np.zeros((19,6))
                a[:,4] = df[4]
            df = pd.DataFrame( a.reshape(19,6) )

            if(countDetectorTotale-1 < 32): 
	
            	df_plot.append([countDetectorTotale-1, df.iloc[18,3]])  
            	print(a.shape, "  a.shape")   
            	print("value and Det: ", countDetectorTotale-1, df.iloc[18,3]) 
            	print(df)

 
            if(countDetectorTotale-1 >= 32): 
	
            	df_plot.append([countDetectorTotale-1, df.iloc[14,3]])   
            	print(a.shape, "  a.shape")  
            	print(countDetectorTotale-1, df.iloc[14,3])  
            	print(df)
