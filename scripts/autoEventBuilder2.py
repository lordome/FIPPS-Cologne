...
for i in range(int(sys.argv[3])):
	...
	cmd1 = "screen -dmS run2" + str(i+100) + "  "
	print(cmd1)
	os.system(cmd1)

	if(int(vecRun[i]) == int(vecRun[i+1])-sub):
		cmd2 = "screen -S run2" + str(i+100) + " -X stuff 'EventBuilder " + confFile + " -batch " + overWrite + " -run " + str(int(vecRun[i])) + "^M'" 
		print(cmd2)
		os.system(cmd2)
		continue

	cmd2 = "screen -S run2" + str(i+100) + " -X stuff 'EventBuilder " + confFile + " -batch " + overWrite + " -run " + str(int(vecRun[i])) + "-" + str(int(vecRun[i+1])-sub) + "^M'" 
	print(cmd2)
	os.system(cmd2)
