...
for i in range(int(sys.argv[3])):
	...
	cmd1 = "screen -dmS runEBaLL" + str(i+100) + "  "
	print(cmd1)
	os.system(cmd1)

	if(int(vecRun[i]) == int(vecRun[i+1])-sub):
		cmd2 = "screen -S runEBaLL" + str(i+100) + " -X stuff 'EventBuilder " + confFile + " -batch " + overWrite + " -run " + str(int(sys.argv[1])) + "-" + str(int(sys.argv[2])) + "^M'" 
		print(cmd2)
		os.system(cmd2)
		continue
